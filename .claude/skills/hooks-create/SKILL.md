...
- **`Stop` / `SubagentStop` only — bound the retries, and do NOT bound them with `stop_hook_active`.** A
  blocking Stop hook does need a bound, or it can block forever: it blocks the stop, the agent works, tries to
  stop again, is blocked again. But `stop_hook_active` is the wrong bound. It is a **boolean, not a counter** —
  it goes true on the very first retry and stays true afterwards, so `exit 0` when it is true means the hook
  blocks **exactly once per session** and then waves every later stop through, checks red or not. That has been
  verified live: a session finished with a failing test sitting in the tree. It looks like a working gate.
  Instead, **count the attempts yourself** — the harness gives no count, so keep a small per-session counter
  (keyed on `session_id`, under `.claude/hooks/.gate-state/`), allow the stop once it passes a cap of about 3,
  and delete the counter as soon as the checks go green. Say plainly in the give-up message that the bound gave
  up rather than the checks passing. `.claude/hooks/stop-gate.sh` in this template is the worked example.
...
- ✅ A blocking `Stop` / `SubagentStop` hook is bounded by a **counter it keeps itself**, not by
  `stop_hook_active` (a boolean that would let every retry after the first through), and it clears that
  counter when the checks pass.
